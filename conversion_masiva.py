...
for i in glob.glob("/gaa/home/data/solar_ecmwf/control/raw/*.nc"):
    program_args = (["python", SCRIPT, i])
    print(" ".join(program_args))
    subprocess.check_call(program_args)
    
for i in glob.glob("/gaa/home/edcastil/scripts/conversion2015/myp/*"):
    #fecha = i[42:][:-8]
    fecha = i[46:][:-8]
    f = datetime.datetime.strptime(fecha, '%Y%m%d')
    matrix_control_dia = (dm.DataMatrix(f,'/gaa/home/edcastil/scripts/conversion2015/myp/',
    '/gaa/home/edcastil/scripts/conversion2015/datamatrix/',ifexists=False,model='ensembles',
    n_ens=1, suffix='.control', tags=dm.nwp_ensembles_tags, delta=0))
    matrix_control_dia.save_matrix(suffix = '.control')
    
#MATRIZ 2013
matrix_control = (dm.DataMatrix(datetime.datetime(2015,12,31),'/gaa/home/edcastil/scripts/conversion2015/myp/','/gaa/home/edcastil/scripts/conversion2015/',ifexists=True,model='ensembles',n_ens = 1, suffix='.control', tags = dm.nwp_ensembles_tags, delta = 364))
