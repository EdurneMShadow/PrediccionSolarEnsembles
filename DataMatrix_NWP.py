...
"""
general scheme for ensembles matrix generation

1. For day in dates
  2. For ensemble in range(n_ens)
     read ensemble file
     reshape
     generate index and columns for that ensemble
     append to last ensemble dataframe as COLUMNS
  append as ROWS to last day dataframe
"""
...
def create_dataframe(year, pathin, pathout, hourly=True, model='deterministic'):
...
aggregated data (source files are 15 minute)."""
    ...
    for t in range(365):
        m = DataMatrix(datetime(year + 1, 1, 1) - timedelta(days=t),
                       pathin, pathout, delta=1, model=model)
...
def get_nwp_deterministic_tags():
    return ['FDIR', 'CDIR', 'TCC',
            'U10', 'V10', 'T2M',
            'SSRD', 'SSR', 'SSRC']


def get_nwp_ensembles_tags():
    return ['FDIR', 'CDIR', 'TCC',
            'U10', 'V10', 'T2M',
            'SSRD', 'SSR']
...
class DataMatrix(object):
    ...
    def __init__(self, date, pathin, pathout, ncoord=2, delta=31, n_ens=50,
                 grid=False, latlons=False, ifexists=False, create=True,
                 dm=False, suffix="", freq='D', model='deterministic'):
        '''
        ...
        if model == 'deterministic':
            self.tags = get_nwp_deterministic_tags()
            nsteps = 24
        elif model == 'ensembles':
            self.tags = get_nwp_ensembles_tags()
            self.n_ens = n_ens
            nsteps = 9
        ...
        self.cols = self.query_cols(latlons=latlons,
                                    tags=self.tags)
        ...
        if os.path.isfile(self.file) and ifexists:
            ...
            if model == 'deterministic':
                ...
                cols = list(np.array([[c + ' {}'.format(ens)
                                       for c in self.cols]
    ...
    def generate_matrix(self, delta, freq, nsteps):
        ...
        for date in pd.date_range(self.date - timedelta(days=delta),
            ...
            datas = shape_data(datas, lon_l, lon_r, lat_l,
                               lat_r, len(self.tags), nsteps)
    ...
    def generate_matrix_ensembles(self, delta, freq, nsteps):
        ...
        for date in pd.date_range(self.date - timedelta(days=delta),
            ...
            for ens in range(self.n_ens):
                fecha = datetime.strftime(date, self.file_format)
                files = self.path + fecha + "_{}.myp.npy".format(ens)
                print(files)
            ...
            if isinstance(self.dataMatrix, bool):
                ...
                if not isinstance(df_ens, bool):
                    self.dataMatrix = pd.concat([self.dataMatrix, df_ens],
                                                join='outer', axis=0)
        ...
        for date in pd.date_range(start_date, end_date, freq=freq):
            if isinstance(ens, bool):
                ...
                if os.path.isfile(files):
                    for hour in pd.date_range(start_date,
                                              end_date +
                                              timedelta(days=1) -
                                              timedelta(hours=1),
                                              freq=freq):
                ...
                files = self.path + \
                    date.strftime(self.file_format) + '_{}.myp.npy'.format(ens)
                if os.path.isfile(files):
                    for hour in pd.date_range(start_date,
                                              end_date +
                                              timedelta(days=1),
